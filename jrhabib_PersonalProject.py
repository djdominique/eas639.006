...
## The very first step is to combine the individual rasters into one larger one. Nappa Valley had their elevation data split into two 
## regions, so we need to do this step to combine them and create one elevation raster for the entire region.

with arcpy.EnvManager(scratchWorkspace=r"E:\Personal_Project\MyProject.gdb", 
                      workspace=r"E:\Personal_Project\MyProject.gdb"):
    arcpy.management.MosaicToNewRaster("Elev_02_400;Elev_02_200", 
                                       r"E:\Personal_Project\MyProject.gdb", 
                                       "CombinedRaster.tif", 
...
out_flow_direction_raster = arcpy.sa.FlowDirection(r"E:\Personal_Project\MyProject.gdb\Fill_Raster", 
                                                   "NORMAL", 
                                                   None, 
                                                   "D8")
...
out_accumulation_raster = arcpy.sa.FlowAccumulation(r"E:\Personal_Project\MyProject.gdb\FlowDir", 
                                                    None, 
                                                    "FLOAT", 
                                                    "D8")
...
arcpy.management.XYTableToPoint("Point.csv", 
                                r"E:\Personal_Project\MyProject.gdb\Point_Location", 
                                "long", 
                                "lat", 
                                None, 
...
out_raster = arcpy.sa.SnapPourPoint(r"E:\Personal_Project\MyProject.gdb\Point_Location", 
                                    r"E:\Personal_Project\MyProject.gdb\FlowAcc", 
                                    1000, 
                                    "OBJECTID")
...
out_raster = arcpy.sa.Watershed(r"E:\Personal_Project\MyProject.gdb\FlowDir", 
                                r"E:\Personal_Project\MyProject.gdb\SnapPoint", 
                                "Value")
...
arcpy.conversion.RasterToPolygon(r"E:\Personal_Project\MyProject.gdb\WatershedRaster", 
                                 r"E:\Personal_Project\MyProject.gdb\Watershed", 
                                 "SIMPLIFY", 
                                 "Value", 
                                 "SINGLE_OUTER_PART", 
                                 None)

# Con tool to define stream raster

streamras = Con("FlowAcc", "1", "", "VALUE > 200") # Value needs to be changed
streamras.save("C:/arcpyexamples/output/outcon.img") # Output location

# Stream Order

outStreamOrder = StreamOrder("stream", "flowdir", "STRAHLER")
outStreamOrder.save("c:/arcpyexamples/output/outstrmordr01")
